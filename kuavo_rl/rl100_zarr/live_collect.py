...
Reuses Kuavo sim/teleop helpers without modifying HIL recording defaults.
"""
...
def run_live_collect_session(config: RL100CollectConfig) -> dict[str, Any]:
    ...
    from kuavo_rl.hil_collect_live import _make_kuavo_gym, _quiet_robot_logs
    ...
    _quiet_robot_logs()
    kuavo_gym = _make_kuavo_gym(deploy_config, max_episode_steps=100_000)
    ...
    allowed.setdefault("reward_button", "B")
    ...
    pc_hub.start()
    pf = pc_hub.preflight(timeout_s=5.0)
    ...
    try:
        while not rospy.is_shutdown():
            ...
            while not rospy.is_shutdown():
                ...
                # Match HIL Quest mapping: → start, ← rerecord, ↓ end session
                ...
                if te is not None and te.is_intervention and te.action is not None:
                    ...
                    env.step(obs["observation.state"])
...
def _record_one_episode(
    *,
    ...
    while True:
        ...
        try:
            pc = pc_hub.get_point_cloud(require_all_enabled=False)
        except Exception as exc:  # noqa: BLE001
    ...
    path = save_episode_npz(
        ...
        point_clouds=pcs,
        result_type=result_type,
        meta={"stop_reason": stop_reason, "task": config.task},
    )
    return LiveEpisodeResult(
        status="saved",
        episode_id=eid,
